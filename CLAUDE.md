...
**Refactored (Phase 1-4 Complete):** TypeScript OOP architecture with dependency injection
...
Scolia WebSocket → Application.handleScoliaMessage()
              → EventOrchestrator.handleThrowDetected()
                  → EffectResolver.resolve() → LightSharkController.triggerExecutor()
                  → SpecialEventDetector.detect() → SoundController.playSound()
                  → KNXController.triggerAction()
                  → GameState updates + persistence

DOM Events:
Playwright → PlaywrightController.on('bust'|'leg-won'|'set-won')
         → Application.handleBustDetected() → SoundController.playSound()
```

**Old Architecture** (Replaced):

- 864-line index.js with all orchestration logic
- 430+ lines of hardcoded if-chains for special events
- Global state variables
- Duplicated logic in simulator.js
...
## Viktiga filer (TypeScript Refactored)

### src/Application.ts (Main bootstrap)

- Instantiates all controllers + orchestrator (dependency injection)
- Manages WebSocket connection to Scolia
- Routes THROW_DETECTED → EventOrchestrator
- Handles Playwright events (bust, leg-won, set-won)
- Graceful shutdown (SIGINT/SIGTERM handlers)

### src/core/EventOrchestrator.ts (Main orchestrator)

- 10-step throw handling: parse → effect → lights → KNX → special events → sound
- Handles THROW_DETECTED, TAKEOUT_FINISHED, TAKEOUT_STARTED
- Coordinates LightShark, Sound, KNX, state updates
- No global variables (all injected)
...
- Maps throws to LightShark executors (priority logic)
- Miss → dark, Bullseye → strobe, T20 → color + strobe, colored segments → color
- Testable in isolation

### src/core/SpecialEventDetector.ts (Data-driven events)

- Replaces 430-line if-chain with strategy pattern
- 8 detector strategies: sumLastN, consecutivePattern, sequentialSegments, etc.
- Reads from `specialEvents.config.ts` (20 declarative events)
- Easy to add new events without code changes

### src/config/specialEvents.config.ts

- Declares all 20 special events (180, 120, 1-2-3, 911, etc.)
- Per-event: detector strategy, parameters, sound, effects
- Non-programmers can add events by editing config

### src/controllers/ (I/O abstraction)

- **LightSharkController.ts** — OSC UDP client
- **SoundController.ts** — Cross-platform (Windows/macOS/Linux)
- **KNXController.ts** — KNX/IP gateway
- **PlaywrightController.ts** — Browser automation + DOM polling for bust/win detection

### src/utils/

- **Logger.ts** — Logging with file rotation
- **SectorParser.ts** — Parse "s14", "d20", "t19" → typed ParsedThrow
- **TypeValidator.ts** — Validate config + payloads (schema validation)

### src/types/index.ts

- Comprehensive TypeScript definitions
- GameThrow, LightSharkExecutor, EffectResolution, etc.
- Controller interfaces (ILightSharkController, ISoundController, etc.)

### src/index.ts (Entry point)

- 6 lines: Create Application, call start()

### src/simulator.ts (Effect tester)

- Tests effect resolution without hardware
- Demonstrates architecture
- Uses Application class (no duplication)
...
## Refactoring Summary (Completed Phase 1-4)
...
1. **Clarity**: 100% self-documenting code (class names, typed methods)
