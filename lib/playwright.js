...
class PlaywrightController extends EventEmitter {
  ...
  async pollDOM() {
    ...
    const state = await this.page.evaluate(() => {
      ...
      // "Finish & View Stats" button — match by text, class hashes changes mellan Scolia-builds
      const finishBtn = Array.from(document.querySelectorAll('button')).some(
        b => (b.innerText || '').trim().includes('Finish & View Stats')
      );
    ...
    if (state.finishBtn && !this.finishBtnTimer) {
      ...
      this.finishBtnTimer = setTimeout(async () => {
        try {
          const finishLocator = this.page.locator('button:has-text("Finish & View Stats")');
          if ((await finishLocator.count()) > 0) {
            this.logger.info('Playwright: Klickar "Finish & View Stats"');
            await finishLocator.first().click();
          }
